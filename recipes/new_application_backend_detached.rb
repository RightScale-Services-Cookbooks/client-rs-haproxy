...
machine_tag "application:active_#{node['client-rs-haproxy']['application_name']}=false" do
 action :create
end
...
log "Running recipe '#{node['client-rs-haproxy']['remote_detach_recipe']}' on all load balancers" +
" with tags 'load_balancer:active_#{node['client-rs-haproxy']['application_name']}=true'..."

execute 'Detach from load balancer(s)' do
  command [
	'rs_run_recipe',
	'--name', node['client-rs-haproxy']['remote_detach_recipe'],
